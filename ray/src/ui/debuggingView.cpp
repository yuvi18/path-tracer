...
DebuggingView::DebuggingView(int x, int y, int w, int h, const char *label)
    : Fl_Gl_Window(x, y, w, h, label), raytracer(0), m_shadingMode(NORMAL),
      m_quality(HIGH_QUALITY), m_lightingMode(SCENE_LIGHTING),
      m_useSceneMaterials(true), m_useSceneTextures(true), m_dirty(true),
      m_showGeometry(true), m_showLights(true), m_showCamera(true),
      m_showAxes(true), m_showNormals(true), m_showVisibilityRays(true),
      m_showReflectionRays(true), m_showRefractionRays(true),
...
void checkGLError() {
  ...
  if (error_flag != GL_NO_ERROR)
    printf("Error: %1s (%i) in %1s.\n", gluErrorString(error_flag), error_flag,
           "method name");
}
